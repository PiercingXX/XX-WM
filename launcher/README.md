...
- **App switcher** (`app_switcher.py`, `toplevel_manager.py`) — full-screen recents over `wlr-foreign-toplevel-management-unstable-v1`: `python-pywayland` on a GLib fd-watch (never blocks the GTK loop), protocol XMLs vendored under `data/`. Last-seen stills plus title; tap → activate, ✕ or swipe-up on a card → close; tap dim / swipe down / back → resume; own surfaces filtered out; graceful empty state when the compositor doesn't offer the protocol.
